        ...
        setEnv(facing, blockEntity.getBlockPos(), poseStack, bufferIn, combinedLight,
                (PoseStack matrixStackIn, MultiBufferSource multiBufferSource, Integer combinedLightIn) -> {
                    ...
                    if (displayMode == CalendarBlock.DisplayMode.SUB_SEASON) {
                        ...
                        drawText(1, translatable.getString(), Optional.ofNullable(subSeason.getSeason().getColor().getColor()).orElse(-1), matrixStackIn, multiBufferSource);
                    } else {
                    ...
                    if (st != SolarTerm.NONE) {
                        ...
                        switch (displayMode) {
                            case YEAR ->
                                    string = Component.translatable("info.eclipticseasons.environment.solar_term.hint2", ClientConfig.GUI.showGregorianYear.get() ? ClientCon.nowGregorianYear : ClientCon.nowSolarYear).getString();
                            case NEXT -> {
