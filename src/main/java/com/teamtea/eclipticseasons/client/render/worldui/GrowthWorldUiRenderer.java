...
public final class GrowthWorldUiRenderer {
    ...
    public static GrowthWorldUiFrameState extractLevelRenderState(RenderLevelStageEvent event) {
        ...
        if (!mc.player.getItemInHand(InteractionHand.MAIN_HAND).is(ItemRegistry.growth_detector.get())) {
            return null;
        }
        ...
        Quaternionf cameraRotation = new Quaternionf(event.getCamera().rotation());
    ...
    public static void renderLevelStage(RenderLevelStageEvent event) {
        if (CommonConfig.Crop.growthDetectorClassicMode.get()) return;
    ...
    private static void fillRect(
        ...
        consumer.vertex(matrix, left, bottom, z).color(color).uv2(light).endVertex();
        consumer.vertex(matrix, right, bottom, z).color(color).uv2(light).endVertex();
        consumer.vertex(matrix, right, top, z).color(color).uv2(light).endVertex();
        consumer.vertex(matrix, left, top, z).color(color).uv2(light).endVertex();
    }
}