    ...
    public void renderStatusBar(GuiGraphics guiGraphics, int screenWidth, int screenHeight, ClientLevel level, LocalPlayer player, String solarDay, long dayTime, double envTemp, int solarTime) {
        ...
        if (showDebug) {
            ...
            infoLines.addKV("Month", Component.translatable("debug_info.eclipticseasons.standard_year_month_day",
                    ClientConfig.GUI.showGregorianYear.get() ? EclipticSeasonsApi.getInstance().getGregorianYear(level) : EclipticSeasonsApi.getInstance().getSolarYears(level),
                ...
                // } else
                {
                    ...
                    if (weatherTarget != null) {
                        ...
                        if (weatherTarget.effect != null && weatherTarget.effect.unwrapKey().isPresent())
                            infoLines.add("Weather Effect: §2" + weatherTarget.effect.unwrapKey().get().location());
