...
public class SuggestedListStringEntry extends ConfigEntry.SpecEntry<List<? extends String>> {
    ...
    public SuggestedListStringEntry(ForgeConfigSpec.ConfigValue<List<? extends String>> spec, Set<WK> possibleValues) {
        super(spec);
    ...
    private void createNewBox(ESModConfigScreen screen, int width, String string, GridLayout.RowHelper helper, int index, List<String> strings) {
        ...
        editBox.setResponder(text -> {
            ...
            ResourceLocation identifier = ResourceLocation.tryParse(text);
            if (identifier != null && (BuiltInRegistries.BLOCK.get(identifier) != Blocks.AIR || text.isEmpty())) {
