...
public class TryModel {
   ...
   public static int getLightFromBlock(GreenHouseCoreBlock block, int stage) {
       if (stage == 0) return LightCoordsUtil.FULL_SKY;
       float cut = 3f / stage;

       switch (block.getSeason()) {
           case SUMMER -> {
               return LightCoordsUtil.pack((int) (15 / cut), 15);
           }
           case WINTER -> {
               return LightCoordsUtil.pack((int) (9 / cut), 15);
           }
           default -> {
               return LightCoordsUtil.pack((int) (12 / cut), 15);
           }
       }
   }
