...
public final class CustomRandomTickHandler {
    ...
    //             && world.getBiome(blockpos).value().getTemperature(pos) >= 0.15F
    //             && WeatherManager.getSnowStatus(world,biome.value(),pos)== WeatherManager.SnowRenderStatus.SNOW_MELT) {
    ...
    //             List<ChunkHolder> list = Lists.newArrayList(level.getChunkSource().chunkMap.getChunks());
