    ...
    public static void processSolarTermsMessage2(SolarTermsMessage solarTermsMessage, IPayloadContext context) {
        context.enqueueWork(() -> {
            SolarHolders.getSaveDataLazy(getUseLevel(context)).ifPresent(data -> {
                ...
                // note 不再需要更新
                // BiomeClimateManager.updateTemperature(ClientCon.getUseLevel(), data.getSolarTerm());
...
//    }
    ...
    public static void processChunkBiomeUpdateMessage(ChunkBiomeUpdateMessage chunkBiomeUpdateMessage, IPayloadContext context) {
        context.enqueueWork(() -> {
            if (getUseLevel(context) != null) {
                ChunkAccess chunk = getUseLevel(context).getChunk(chunkBiomeUpdateMessage.x, chunkBiomeUpdateMessage.z, ChunkStatus.FULL, false);
                if (chunk != null) {
