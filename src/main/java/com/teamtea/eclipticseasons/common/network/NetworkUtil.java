    ...
    public static void processSolarTermsMessage2(SolarTermsMessage solarTermsMessage, IPayloadContext context) {
        context.enqueueWork(() -> {
            SolarHolders.getSaveDataLazy(getUseLevel(context)).ifPresent(data -> {
                ...
                if (solarTerm != old) {
                    NeoForge.EVENT_BUS.post(new SolarTermChangeEvent(old, solarTerm, getUseLevel(context), data.getSolarTermsDay()));
                    ClientCon.getAgent().setTermChange(true);
                }
                // note 不再需要更新
                // BiomeClimateManager.updateTemperature(getUseLevel(context), data.getSolarTerm());
