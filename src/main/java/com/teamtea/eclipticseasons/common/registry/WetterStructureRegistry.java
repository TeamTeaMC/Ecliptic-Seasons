...
public class WetterStructureRegistry {
    ...
    public static void bootstrap(BootstapContext<WetterStructure> context) {
        var blockHolderGetter = context.lookup(Registries.BLOCK);
        context.register(BUBBLE_COLUMN_ON_MAGMA, new WetterStructure(0.75f, 4,600,true,Optional.of(new FakeBlockPredicate( Optional.of(HolderSet.direct(Blocks.BUBBLE_COLUMN.builtInRegistryHolder())),Optional.empty())),  List.of(
                new PosAndBlockStateCheck(Vec3i.ZERO.below(),
                       new  FakeBlockPredicate(Optional.of(HolderSet.direct(Blocks.MAGMA_BLOCK.builtInRegistryHolder())),Optional.empty()))
        )));
    }
}
