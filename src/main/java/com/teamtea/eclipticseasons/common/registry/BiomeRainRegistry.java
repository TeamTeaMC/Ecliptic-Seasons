        ...
        for (int i = 0; i < TemperateRain.collectValues().length; i++) {
            ...
            solarTermValueMap.solarTermMap().get().put(
                    solarTerm, List.of(new CustomRainBuilder.Weather(
                            se ? Optional.of(ServerLevel.RAIN_DURATION) : Optional.empty(),
                            se ? Optional.of(ServerLevel.RAIN_DELAY) : Optional.empty(),
                            se ? Optional.of(ServerLevel.THUNDER_DURATION) : Optional.empty(),
