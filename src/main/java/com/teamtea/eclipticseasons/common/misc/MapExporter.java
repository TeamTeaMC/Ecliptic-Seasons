    ...
    public static int exportMap(CommandSourceStack source, BlockPos pos) {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                ...
                if (biome.is(Biomes.THE_VOID)) {
                    ...
                    color = new Color(RandomSource.create(biome.unwrapKey().get().location().hashCode()).nextInt(256 * 256 * 256));
                }
        ...
        for (Map.Entry<Holder<Biome>, Color> holderColorEntry : hashSet.entrySet()) {
            ...
            graphics2D.drawString(
                    holderColorEntry.getKey() == null ? "not load" : holderColorEntry.getKey().unwrapKey().get().location() + "," + Component.translatable(Util.makeDescriptionId("biome", holderColorEntry.getKey().unwrapKey().get().location())).getString(), 5,
                    20 * (++i));
        }
