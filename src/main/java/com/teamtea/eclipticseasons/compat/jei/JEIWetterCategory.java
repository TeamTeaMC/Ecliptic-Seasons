...
public class JEIWetterCategory implements IRecipeCategory<WetterStructure> {
    ...
    @Override
    public @NotNull RecipeType<WetterStructure> getRecipeType() {
    ...
    @Override
    public void draw(WetterStructure recipe, @NotNull IRecipeSlotsView recipeSlotsView, @NotNull GuiGraphics
            guiGraphics, double mouseX, double mouseY) {

        guiGraphics.pose().pushPose();

        //patch offset
        guiGraphics.pose().translate(-25, 0, 0);
                ...
                if (blockHolder.isBound()) {
                    ...
                    try {
                        ...
                        if (!blockState.getFluidState().isEmpty())
                            BlockGuiRenderUtil.renderFluidInGui(guiGraphics, blockState.getFluidState(), 1, 0, -0.4f, 1f);
                        // guiGraphics.flush();
        ...
        guiGraphics.pose().popPose();
        //patch offset
        guiGraphics.pose().translate(-34, 0, 0);
        ...
        Font font = Minecraft.getInstance().font;
        {
...

            {
                guiGraphics.pose().pushPose();
                guiGraphics.pose().scale(.75f, .75f, .75f);
                String timeString =  ((int) (recipe.lasting_time() / 20)) + "s";
